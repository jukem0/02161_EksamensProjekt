...
public class bestem_budgetteret_tid_steps {
    ...
    public void medarbejderen_bestemmer_budgetteret_tid_til_som_positivt_decimaltal_eller_heltal_som_slutter_i_og_varer(
        ...
        createActivitysteps.projects.get(0).addActivity(aktivitetsNavn, budgetTime, new Week(endWeek, Year.now().getValue()), amountWeek);
        System.out.println(createActivitysteps.projects.get(0).getActivity(0).getActivityName());
    }
    ...
    public void tildel_mængde_af_budgetteret_tid_rundet_op_til_naermeste_halve(double time) {
        assertEquals(time, createActivitysteps.projects.get(0).getActivity(0).getBudgetTime(), 0.25);
    }
