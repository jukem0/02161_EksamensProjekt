...
public class tildel_projektleder_steps{
    ...
    public void udpegning(String udpeger, String medarbejder, String projekt){
        ...
        statusconfirm = true;
            ...
            assert (udpeger.appointPL(projekt, medarbejder)) : "Tildeling mislykkedes: Medarbejder " + medarbejder + " er allerede projektleder for projekt " + projekt;
            statusconfirm = true;
        }
        else if(status.erProjektLeder() && !status.projektnavn().equals(projekt)){
            statusconfirm = false;
    ...
    public void tildelProjektlederSuccess(String medarbejder, String projekt) {
        ...
        status = Main.erProjektleder(projekt, medarbejder);
        assert (status.erProjektLeder() && status.projektnavn().equals(projekt)) : "Tildeling mislykkedes: Medarbejder " + medarbejder + " er ikke projektleder for projekt " + projekt;
    }
    ...
    public void medarbejderErProjektlederForAndetProjekt(String medarbejder, String projekt) {
        ...
        "Der er gået noget galt: enten er medarbejder " + medarbejder + " ikke projektleder for noget projekt, eller så er de projektleder for det samme projekt " + projekt;
        statusconfirm = false;
    }
    ...
    public void tildelPLFejler(String medarbejder) {
        if(statusconfirm == false){
            assert (!status.erProjektLeder()) : "Handling fejler med fejlbesked: Medarbejder " + medarbejder + " er allerede projektleder for projekt " + status.projektnavn();
        }
    }
